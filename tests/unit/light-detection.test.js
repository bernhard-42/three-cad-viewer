 ...
 */
function createHdrWithSpot(width, height, bgValue, spotX, spotY, spotValue, spotRadius = 2) {
...
describe('detectDominantLights — spot detection', () => {
  test('detects a single bright spot (Float32Array)', () => {
    ...
    const len = Math.sqrt(light.direction[0] ** 2 + light.direction[1] ** 2 + light.direction[2] ** 2);
  ...
  test('colored spot preserves color ratios', () => {
    ...
    for (let i = 0; i < data.length; i += 3) {
      data[i] = 0.05; data[i + 1] = 0.05; data[i + 2] = 0.05;
    }
    // Red spot at center
    const cx = 64, cy = 32, r = 3;
    ...
    expect(result.lights[0].color[0]).toBeGreaterThan(result.lights[0].color[1]);
  });
});
