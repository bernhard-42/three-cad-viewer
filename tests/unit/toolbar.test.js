...
describe("Toolbar - minimize/maximize", () => {
  ...
  test("minimize skips disabled feature buttons", () => {
    ...
    const propertiesBtn = new Button(
      "light",
      "properties",
      "Properties",
      vi.fn(),
...
describe("Button", () => {
  ...
  test("highlight adds/removes class", () => {
    ...
    expect(button.html.firstChild.classList.contains("tcv_btn_highlight")).toBe(
      true,
    );
...
describe("ClickButton", () => {
  ...
  test("set updates state and toggles class", () => {
    ...
    expect(button.state).toBe(true);
    expect(button.html.children[0].classList.contains("tcv_btn_click2")).toBe(
      true,
    );
...
describe("ClickButton - Dropdown", () => {
  test("creates dropdown with checkboxes", () => {
    const action = vi.fn();
    const button = new ClickButton("light", "grid", "Grid", action, false, [
      "xy",
      "xz",
      "yz",
    ]);
  ...
  test("extractIdFromName extracts grid id", () => {
    const button = new ClickButton("light", "grid", "Grid", vi.fn(), false, [
      "xy",
    ]);
