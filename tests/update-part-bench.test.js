...
describe("Viewer.updatePart – correctness", () => {
  ...
  test("throws if path does not exist", async () => {
    ...
    expect(() =>
      ctx.viewer.updatePart("/Group/NoSuch", createBox("NoSuch")),
...
describe("Viewer.updatePart – benchmark", () => {
    ...
    // --- skipBounds batch (what real callers should use) ---
    {
      ...
      for (let iter = 0; iter < ITERS; iter++) {
        ...
        for (const p of paths) {
          viewer.updatePart(
            p,
            createBox(p.split("/").pop(), parseOffset(p), h),
            { skipBounds: true },
          );
        }
        viewer.updateBounds();
  ...
  test("timing comparison: 10, 30, 50 parts", async () => {
    ...
    const colW = 12;
    const labelW = 28;
    const header = partCounts
      .map((n) => `${n} Parts`.padStart(colW))
      .join(" │");

    console.log(`\n  updatePart benchmark  (avg ms per pass, 3 iterations)`);
    console.log(
      `  ┌${"─".repeat(labelW)}┬${partCounts.map(() => "─".repeat(colW)).join("─┬─")}─┐`,
    );
    ...
    const speedupCols = partCounts
      ...
      .join(" │");
    console.log(
      `  │ ${"Speedup (batch vs r+a)".padEnd(labelW - 1)}│${speedupCols} │`,
    );
    ...
    console.log(
      `  │ ${"Method".padEnd(labelW - 1)}│${partCounts.map((n) => `${n} Parts`.padStart(colW)).join(" │")} │`,
    );
    console.log(
      `  ├${"─".repeat(labelW)}┼${partCounts.map(() => "─".repeat(colW)).join("─┼─")}─┤`,
    );
    ...
    const refDirectCols = partCounts
      .map((n) => `${refDirect[n]} ms`.padStart(colW))
      .join(" │");
    const refRemoveCols = partCounts
      .map((n) => `${refRemove[n]} ms`.padStart(colW))
      .join(" │");
