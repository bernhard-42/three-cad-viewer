 ...
 */
export function almostEqualDiff(
  actual,
  expected,
  tolerance = 1e-6,
  path = "root",
  ...
  if (actualType !== expectedType) {
    diffs.push(
      `${path}: type mismatch (expected ${expectedType}, got ${actualType})`,
    );
    return diffs;
  }
  ...
  if (actualType === "number") {
    ...
    if (diff > tolerance) {
      diffs.push(
        `${path}: ${actual} !== ${expected} (diff: ${diff.toExponential(2)}, tolerance: ${tolerance})`,
      );
    }
    return diffs;
  }
  ...
  if (Array.isArray(actual) && Array.isArray(expected)) {
    if (actual.length !== expected.length) {
      diffs.push(
        `${path}: array length mismatch (expected ${expected.length}, got ${actual.length})`,
      );
      return diffs;
    }

    for (let i = 0; i < actual.length; i++) {
      const subDiffs = almostEqualDiff(
        actual[i],
        expected[i],
        tolerance,
        `${path}[${i}]`,
  ...
  if (ArrayBuffer.isView(actual) && ArrayBuffer.isView(expected)) {
    if (actual.constructor.name !== expected.constructor.name) {
      diffs.push(
        `${path}: typed array type mismatch (expected ${expected.constructor.name}, got ${actual.constructor.name})`,
      );
      return diffs;
    }
    if (actual.length !== expected.length) {
      diffs.push(
        `${path}: typed array length mismatch (expected ${expected.length}, got ${actual.length})`,
      );
      return diffs;
    }
    ...
    for (let i = 0; i < actual.length; i++) {
      ...
      if (diffs.length > 10) {
        diffs.push(
          `${path}: ... (${actual.length - i - 1} more elements not checked)`,
        );
        break;
      }
    }
  ...
  if (actualType === "object") {
    ...
    if (actual.constructor.name !== expected.constructor.name) {
      diffs.push(
        `${path}: constructor mismatch (expected ${expected.constructor.name}, got ${actual.constructor.name})`,
      );
      return diffs;
    }
...
export function toAlmostEqual(actual, expected, tolerance = 1e-6) {
  ...
  if (equal) {
    return {
      pass: true,
      message: () =>
        `Expected values not to be almost equal (tolerance: ${tolerance})`,
    };
  } else {
    ...
    return {
      pass: false,
      message: () =>
        `Expected values to be almost equal (tolerance: ${tolerance}):\n  ${diffReport}${truncated}`,
    };
  }
}
