...
describe('decode-instances', () => {
  test('isInstancedFormat detects instanced data', () => {
    ...
    expect(isInstancedFormat({ instances: 'not-array', shapes: {} })).toBe(false);
  });
  ...
  test('rejects unknown dtype', () => {
    const badInstance = makeEncodedInstance(new Float32Array([0]), new Uint32Array([0]));
    badInstance.vertices.dtype = 'float64';

    const data = {
      instances: [badInstance],
      shapes: { id: 'r', name: 'R', loc: [[0, 0, 0], [0, 0, 0, 1]], parts: [] },
    };
...
describe('Studio mode switching', () => {
  ...
  test('resetStudio resets all keys to defaults', () => {
    ...
    expect(viewer.getStudioTextureMapping()).toBe(defaults.studioTextureMapping);
    expect(viewer.getStudioShadowIntensity()).toBe(defaults.studioShadowIntensity);
  });
