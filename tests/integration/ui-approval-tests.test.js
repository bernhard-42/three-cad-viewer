...
describe("UI-Level Approval Tests - Display/Viewer Integration", () => {
  ...
  describe("Display Control Buttons", () => {
    ...
    test("UI: toggle grid button", async () => {
      ...
      expect({ gridOnState, gridOffState, gridOnAgainState }).toMatchSnapshot(
        "ui-grid-toggle",
      );
    });
  });
  ...
  describe("Material Control Buttons", () => {
    test("UI: toggle transparent button", async () => {
      ...
      expect({
        ...
        transparentState,
        opaqueAgainState,
      }).toMatchSnapshot("ui-transparent-toggle");
    });
    ...
    test("UI: toggle black edges button", async () => {
      ...
      expect({
        ...
        blackEdgesOnState,
        blackEdgesOffState,
      }).toMatchSnapshot("ui-blackedges-toggle");
    });
  });
  ...
  describe("Rendering Mode Buttons", () => {
    test("UI: toggle explode mode button", async () => {
      ...
      if (
        display.clickButtons["explode"] &&
        typeof viewer.explode === "function"
      ) {
        ...
        expect({ compactState, skipped: true }).toMatchSnapshot(
          "ui-explode-toggle",
        );
      }
    });
  ...
  describe("Clipping Control Buttons", () => {
    test("UI: toggle clipping button", async () => {
      ...
      if (display.clickButtons["clip"]) {
        ...
        expect({ initialState, clippingOnState }).toMatchSnapshot(
          "ui-clipping-toggle",
        );
      }
    });
  ...
  describe("Material Property Controls", () => {
    test("UI: adjust metalness slider", async () => {
      ...
      expect(adjustedState.scene.sampleMaterial.metalness).not.toBe(
        initialMetalness,
      );

      expect({ initialState, adjustedState }).toMatchSnapshot(
        "ui-metalness-slider",
      );
    });
    ...
    test("UI: adjust roughness slider", async () => {
      ...
      expect(adjustedState.scene.sampleMaterial.roughness).not.toBe(
        initialRoughness,
      );
    ...
    test("UI: adjust ambient light slider", async () => {
      ...
      expect({ initialState, brightState }).toMatchSnapshot(
        "ui-ambient-light-slider",
      );
    });
  ...
  describe("Complex UI Workflows", () => {
    ...
    test("UI workflow: adjust multiple materials then toggle views", async () => {
      ...
      expect({ materialsAdjusted, frontView, topView }).toMatchSnapshot(
        "ui-materials-views-workflow",
      );
    });
    ...
    test("UI workflow: rapid UI interactions (stress test)", async () => {
      ...
      transparentBtn.set(!transparentBtn.state);
      transparentBtn.action(transparentBtn.name, transparentBtn.state); // ON
      blackEdgesBtn.set(!blackEdgesBtn.state);
      blackEdgesBtn.action(blackEdgesBtn.name, blackEdgesBtn.state); // ON
      perspectiveBtn.set(!perspectiveBtn.state);
      perspectiveBtn.action(perspectiveBtn.name, perspectiveBtn.state); // Switch
      axesBtn.set(!axesBtn.state);
      axesBtn.action(axesBtn.name, axesBtn.state); // OFF
      gridBtn.set(!gridBtn.state);
      gridBtn.action(gridBtn.name, gridBtn.state); // OFF
      display.buttons["top"].action("top", false); // Change view
      axesBtn.set(!axesBtn.state);
      axesBtn.action(axesBtn.name, axesBtn.state); // ON
      transparentBtn.set(!transparentBtn.state);
      transparentBtn.action(transparentBtn.name, transparentBtn.state); // OFF
  ...
  describe("Display-Viewer Integration Verification", () => {
    test("UI buttons correctly propagate to viewer state", async () => {
      ...
      const tests = [
        ...
        {
          button: "transparent",
          check: (state) => state.scene.sampleMaterial.transparent === true,
        },
        {
          button: "perspective",
          check: (state) => state.camera.ortho === false,
        },
      ];
