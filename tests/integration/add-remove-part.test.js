...
function createTestPart(name, offset = 2) {
  return {
    ...
    shape: {
      ...
      triangles: [
        1, 2, 0, 1, 3, 2, 5, 4, 6, 5, 6, 7, 11, 8, 9, 11, 10, 8, 15, 13, 12, 15,
        12, 14, 19, 16, 17, 19, 18, 16, 23, 21, 20, 23, 20, 22,
      ],
      normals: [
...
describe("Viewer - addPart", () => {
  ...
  test("throws if parent group does not exist", async () => {
    ...
    expect(() =>
      viewer.addPart("/NonExistent", createTestPart("Orphan")),
...
describe("Viewer - removePart", () => {
  ...
  test("removes a subtree and all its children", async () => {
    ...
    expect(
      viewer.rendered.nestedGroup.groups["/Group/Shelf/Left"],
    ).toBeDefined();
    ...
    expect(
      viewer.rendered.nestedGroup.groups["/Group/Shelf/Right"],
    ).toBeUndefined();
  });
...
describe("Viewer - addPart/removePart round-trip", () => {
  ...
  test("scene returns to original state after add then remove", async () => {
    ...
    const statesBefore = viewer.getStates();
    const groupCountBefore = Object.keys(
      viewer.rendered.nestedGroup.groups,
    ).length;
    ...
    const statesAfter = viewer.getStates();
    const groupCountAfter = Object.keys(
      viewer.rendered.nestedGroup.groups,
    ).length;
